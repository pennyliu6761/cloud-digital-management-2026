...
學號格式錯誤：    =COUNTIF(dirty_data!A2:A6, "abc") （簡化版）

重複學號筆數：    =SUMPRODUCT((COUNTIF(dirty_data!A2:A6,
                   dirty_data!A2:A6)>1)*1)

Email 包含 @ 的筆數：=COUNTIF(dirty_data!C2:C6, "*@*")

資料完整率：      =TEXT((COUNTA(dirty_data!B2:B6)/
                   COUNTA(dirty_data!A2:A6)), "0%")
```

<!-- 📸 截圖：quality_dashboard 工作表顯示資料品質指標 -->
